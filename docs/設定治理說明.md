...
- `AuthProperties`：接管 `app.auth.demo.*`、`app.auth.front.demo.*`、`app.auth.guard.enabled` 與 **Guard 路徑**（`admin-path-prefixes`、`front-path-prefixes`、`front-exclude-prefixes`），詳見 權限守衛與RBAC說明.md。
- `CorsProperties`：接管 `app.cors.allowed-origins`、`app.cors.allow-credentials`
- `StorageProperties`：定義 `app.storage.*`（local / s3）型別化骨架
- `AuditProperties`：定義 `app.audit.*`（啟用、sink=console/db、body、遮罩）型別化；稽核已實作，依 `app.audit.sink` 輸出至 log 或 DB（見 稽核日誌說明.md）。
...
1. 實作或擴充 `StorageService` 時，統一僅讀取 `StorageProperties`。
2. Audit Log 已依 `AuditProperties` 實作（console/db）；若需記錄 body 或遮罩，可擴充 `AuditProperties` 的 include* 與 maskFields。
